...
class DB:
    ...
    async def set_kv(cls, guild_id:int, key:str, value):
        conn = await cls.get_conn()
        v = json.dumps(value)
        await conn.execute("INSERT OR REPLACE INTO kv_store (guild_id, key, value) VALUES (?, ?, ?)", (guild_id, key, v))
        await conn.commit()

    @classmethod
    async def add_modlog(cls, guild_id:int, action:str, moderator_id:int, target_id:int, reason:str, ts:int):
        conn = await cls.get_conn()
        await conn.execute("INSERT INTO modlog (guild_id, action, moderator_id, target_id, reason, timestamp) VALUES (?, ?, ?, ?, ?, ?)",
                           (guild_id, action, moderator_id, target_id, reason, ts))
        await conn.commit()

    @classmethod
    async def get_modlog(cls, guild_id:int, limit:int=50):
        conn = await cls.get_conn()
        ...
        await cur.close()
        return [{"id":r[0],"action":r[1],"moderator_id":r[2],"target_id":r[3],"reason":r[4],"timestamp":r[5]} for r in rows]
